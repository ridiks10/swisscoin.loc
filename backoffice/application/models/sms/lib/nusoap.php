...
//$GLOBALS['_transient']['static']['nusoap_base']->globalDebugLevel = 9;//unc
...
class nusoap_base {
    ...
    function nusoap_base() {
        //$this->debugLevel = $GLOBALS['_transient']['static']['nusoap_base']->globalDebugLevel;//unc
    ...
    function expandQname($qname) {
        // get element prefix
        //if (strpos($qname, ':') && !ereg('^http://', $qname)) {//unc
...
class soap_transport_http extends nusoap_base {
    ...
    function soap_transport_http($url, $curl_options = NULL, $use_curl = false) {
        ...
        //ereg('\$Revisio' . 'n: ([^ ]+)', $this->revision, $rev);//unc
...
//		}
    ...
    function setEncoding($enc = 'gzip, deflate') {
        if (function_exists('gzdeflate')) {
            ...
            //set_magic_quotes_runtime(0);//unc deprecated
            // deprecated
            $this->encoding = $enc;
        }
    }
    ...
    function getResponse() {
        ...
        if ($this->io_method() == 'socket') {
            ...
            while (!isset($lb)) {
                ...
                // remove 100 headers
                //if (isset($lb) && ereg('^HTTP/1.1 100',$data)) {//unc
...
class wsdl extends nusoap_base {
    ...
    function end_element($parser, $name) {
        // unset schema status
       // if (/* ereg('types$', $name) || */ ereg('schema$', $name)) {//unc
...
class nusoap_parser extends nusoap_base {
    ...
    function start_element($parser, $name, $attrs) {
        ...
        foreach ($attrs as $key => $value) {
            ...
            // if ns declarations, add to class level array of valid namespaces
            if ($key_prefix == 'xmlns') {
               // if (ereg('^http://www.w3.org/[0-9]{4}/XMLSchema$', $value)) {//unc
...
class nusoap_client extends nusoap_base {
    ...
    function loadWSDL() {
        ...
        $this->wsdl = new wsdl('', $this->proxyhost, $this->proxyport, $this->proxyusername, $this->proxypassword, $this->timeout, $this->response_timeout, $this->curl_options, $this->use_curl); //unc add &
    ...
    function send($msg, $soapaction = '', $timeout = 0, $response_timeout = 30) {
        ...
        switch (true) {
            // http(s)
            //case ereg('^http', $this->endpoint)://unc
            case preg_match('/^http/',$this->endpoint):
               ...
               // if (ereg('^http:', $this->endpoint)) {//unc
    ...
    function parseResponse($headers, $data) {
        ...
        if (strpos($headers['content-type'], '=')) {
            $enc = str_replace('"', '', substr(strstr($headers["content-type"], '='), 1));
            $this->debug('Got response encoding: ' . $enc);
            //if (eregi('^(ISO-8859-1|US-ASCII|UTF-8)$', $enc)) {//unc
