...
//        return $row->cash_account;
        $this->load->model('ewallet_model');
        return (double) $this->ewallet_model->getCommission( $user_id, Date( 'Y-m-d', strtotime( $this->ewallet_model->getJoiningDate( $user_id ) ) ), Date( 'Y-m-d' ) );
...
//    }
    ...
    public function insertBalAmountDetails($from_user_id, $to_user_id, $trans_amount, $amount_type = '', $transaction_concept = '', $trans_fee = 0) {
    ...
    public function updateBalanceAmountDetailsFrom($from_user_id, $trans_amount) {
        ...
        $this->db->set('cash_account', 'ROUND(cash_account - ' . $trans_amount . ',2)', FALSE);
	...
	public function getCommission2( $user_id, $from_date = null, $to_date = null, array $pagination = null, $output = false ) {
		$isTimeQuery = false;
		...
		$this->db->select( "la.cash_account as total_amount" );
		...
		$leg_amount = $this->db->select_string();


		$this->db->select('ftd.id as id');
		$this->db->select("IF(  ftd.amount_type =  'user_credit', (ftd.amount +  ftd.trans_fee),  ftd.amount ) AS total_amount" );
		$this->db->select("ftd.amount_type as amount_type");
		$this->db->select( "UNIX_TIMESTAMP( ftd.date ) as full_date" );
		$this->db->from('fund_transfer_details as ftd');
		$this->db->where('ftd.to_user_id', $user_id );
		if( $isTimeQuery ) {
			$this->db->where("ftd.date BETWEEN '$_start' AND '$_finish'");
		}
		$fund_transfers1 = $this->db->select_string();

		$this->db->select( "pp.pin_id as id" );
		$this->db->select( "pp.pin_amount as total_amount" );
		$this->db->select('\'pin_purchased\' as amount_type', false);
		$this->db->select( "UNIX_TIMESTAMP( pp.pin_uploded_date ) as full_date" );
		$this->db->from( 'pin_purchases as pp' );
		$this->db->where( 'pp.allocated_user_id', $user_id );
		$this->db->where( 'pp.purchase_status', 'yes' );
		if( $isTimeQuery ) {
			$this->db->where("pp.pin_alloc_date BETWEEN '$_start' AND '$_finish'");
		}
		$pin_purchases = $this->db->select_string();

		$this->db->select('ap.paid_id as id');
		$this->db->select('ap.paid_amount as total_amount');
		$this->db->select('ap.paid_type as amount_type' );
		$this->db->select( "UNIX_TIMESTAMP( ap.paid_date ) as full_date" );
		$this->db->from('amount_paid as ap');
		$this->db->where('ap.paid_user_id', $user_id);
		$this->db->where_in('ap.paid_type', ['released', 'release_comission'] );
		if( $isTimeQuery ) {
			$this->db->where("ap.paid_date BETWEEN '$_start' AND '$_finish'");
		}
		$amount_paid = $this->db->select_string();
		...
		$this->db->where_in('epd.payed_account', [ 'cash', 'ewallet' ] );
		if( $isTimeQuery ) {
		...
		$sql = sprintf( " %s UNION ALL %s UNION ALL %s  UNION ALL %s  UNION ALL %s ORDER BY full_date DESC, id DESC" , $leg_amount, $fund_transfers1, $pin_purchases, $amount_paid, $ewallet_details );
		...
		foreach ( $details as &$detail ) {
			$balance += ( round( $detail['total_amount'], 2 ) * ( in_array( $detail['amount_type'], [
					'user_debit', 'admin_debit', 'released', 'pin_purchased', 'annual_fee', 'registration', 'repurchase'
				] ) ? -1 : 1 ) );
	...
	public function getCommissionDetails2( $user_id, $from_date = null, $to_date = null, array $pagination = null, $initial_balance = 0 ) {
		$isTimeQuery = false;
		if( ! is_null( $from_date ) && ! is_null( $to_date ) ) {
			$_start  = DateTime::createFromFormat( 'Y-m-d', $from_date )->setTime( 0, 0 )->format('Y-m-d H:i:s');
			$_finish = DateTime::createFromFormat( 'Y-m-d', $to_date )->setTime( 23, 59 )->format('Y-m-d H:i:s');
		...
		$this->db->select("NULL as transaction_concept", false );
		$this->db->from( "leg_amount as la" );
		$this->db->join( 'amount_type as at', 'at.db_amt_type = la.amount_type', 'inner' );
		...
		$this->db->select("fi.user_name as from_user_name");
		$this->db->select( "UNIX_TIMESTAMP( ftd.date ) as full_date" );
		$this->db->select( "STR_TO_DATE( ftd.date, ('%Y-%m-%d') ) as date" );
		$this->db->select("ftd.transaction_concept as transaction_concept" );
		$this->db->from('fund_transfer_details as ftd');
		$this->db->join( 'ft_individual as fi', 'fi.id = ftd.from_user_id', 'left' );
		$this->db->where('ftd.to_user_id', $user_id );
		if( $isTimeQuery ) {
			$this->db->where("ftd.date BETWEEN '$_start' AND '$_finish'");
		}
		...
		$this->db->select('fi.user_name as from_user_name');
		$this->db->select( "UNIX_TIMESTAMP( pp.pin_uploded_date ) as full_date" );
		$this->db->select( "STR_TO_DATE( pp.pin_uploded_date, ('%Y-%m-%d') ) as date" );
		$this->db->select("NULL as transaction_concept", false );
		$this->db->from( 'pin_purchases as pp' );
		$this->db->join( 'ft_individual as fi', 'fi.id = pp.generated_user_id', 'left' );
		$this->db->where( 'pp.allocated_user_id', $user_id );
		$this->db->where( 'pp.purchase_status', 'yes' );
		if( $isTimeQuery ) {
			$this->db->where("pp.pin_alloc_date BETWEEN '$_start' AND '$_finish'");
		...
		$this->db->select('ap.paid_amount as total_amount');
		$this->db->select('ap.paid_type as amount_type' );
		$this->db->select('\'Admin\' as from_user_name', false );
		$this->db->select( "UNIX_TIMESTAMP( ap.paid_date ) as full_date" );
		$this->db->select( "STR_TO_DATE( ap.paid_date, ('%Y-%m-%d') ) as date" );
		$this->db->select("NULL as transaction_concept", false );
		$this->db->from('amount_paid as ap');
		$this->db->where('ap.paid_user_id', $user_id);
		$this->db->where_in('ap.paid_type', ['released', 'release_comission'] );
		...
		foreach ( $details as &$detail ) {
			$balance += ( round( $detail['total_amount'], 2 ) * ( in_array( $detail['amount_type'], [
					'user_debit', 'admin_debit', 'released', 'pin_purchased', 'annual_fee', 'registration', 'repurchase', 'release_comission'
				] ) ? -1 : 1 ) );
			$detail['balance'] = $balance;
		}
    ...
    public function getCommissionDetails($user_id, $from_date = '', $to_date = '', $limit = 0, $page = 0) {
        ...
        $this->db->select('amount_payable,total_amount,amount_type,cash_account,date_of_submission,from_id');
        ...
        $query = $this->db->get();
        foreach ($query->result_array() as $row) {
            $details[$i]['total_amount'] = $row['cash_account'];
        ...
        $this->db->select('amount as total_amount,date,amount_type,from_user_id,to_user_id,trans_fee,transaction_concept');
        $this->db->from('fund_transfer_details');
        $this->db->where('to_user_id', $user_id);
        if ($from_date != '' && $to_date != '') {
        ...
        if ($query1->num_rows() != 0) {
            foreach ($query1->result_array() as $row) {
                $details[$i]['total_amount'] = $row['total_amount'] + $row['trans_fee'];
                if ($row['amount_type'] == 'user_credit') {
                    $details[$i]['total_amount'] = $row['total_amount'];
                }
                $details[$i]['amount_type'] = $row['amount_type'];
                $from_user_id = $row['from_user_id'];
                $from_user_name = $this->validation_model->IdToUserName($from_user_id);
                $details[$i]['from_user_name'] = $from_user_name;
                $details[$i]['date'] = date('Y-m-d', strtotime($row['date']));
                $details[$i]['transaction_concept'] = $row['transaction_concept'];
        ...
        if ($pin_status) {
            $this->db->select('pin_prod_refid,pin_uploded_date,pin_amount,generated_user_id');
            ...
            foreach ($query2->result_array() as $row) {
                $details[$i]['total_amount'] = $row['pin_amount'];
                $details[$i]['amount_type'] = 'pin_purchased';
                $details[$i]['date'] = date('Y-m-d', strtotime($row['pin_uploded_date']));
                $details[$i]['full_date'] = strtotime($row['pin_uploded_date']);
                $details[$i]['from_user_name'] = $this->validation_model->IdToUserName($row['generated_user_id']);
                $i++;
            }
        }
        ...
        $this->db->where_in('paid_type', ['released', 'release_comission'] );
        if ($from_date != '' && $to_date != '') {
        ...
        foreach ($query3->result_array() as $row) {
            ...
            $details[$i]['amount_type'] = ( $row['paid_type'] === 'released' ) ? 'payout_released' :  $row['paid_type'];
            $details[$i]['date'] = date('Y-m-d', strtotime($row['paid_date']));
            $details[$i]['full_date'] = strtotime($row['paid_date']);
            $details[$i]['from_user_name'] = "Admin";
            $i++;
        }
        $a_type=array('cash','ewallet');
        $this->db->select('user_id,used_amount,used_for,date');
        $this->db->from('ewallet_payment_details');
        $this->db->where('used_user_id', $user_id);
        $this->db->where_in('payed_account', $a_type);
        if ($from_date != '' && $to_date != '') {
            $this->db->where("date BETWEEN '$start' AND '$end'");
        }
        $query4 = $this->db->get();
        ...
        function sort_by_date($a, $b){
            return $a['full_date'] == $b['full_date'] ? 0 : ($a['full_date'] < $b['full_date'] ? 1 : -1);
        }
        usort($details, 'sort_by_date');

        if($limit)
            $details = array_slice($details, $page * $limit, $limit);

        $balance = 0;
        foreach ($details as &$detail) {
            $balance += (round($detail['total_amount'], 2) * (in_array($detail['amount_type'], [
                    'user_debit', 'admin_debit', 'payout_released', 'pin_purchased', 'annual_fee', 'registration', 'repurchase', 'release_comission'
                ]) ? -1 : 1));
            $detail['balance'] = $balance;
        }
    ...
    public function getCommissionDetailsCount($user_id, $from_date = '', $to_date = '') {
        ...
        $i += $this->db->get()->first_row('array')['count'];
        ...
        if ($pin_status) {$this->db->select('count(*) as count');
            $this->db->from('pin_purchases');
            $this->db->where('allocated_user_id', $user_id);
            $this->db->where('purchase_status', 'yes');
            if ($from_date != '' && $to_date != '') {
    ...
    public function getCommission($user_id, $from_date, $to_date) {
        ...
        $balance = 0;
        $this->db->select('total_amount,amount_type,cash_account,date_of_submission,from_id');
        $this->db->from('leg_amount');
        $this->db->where('user_id', $user_id);
        if ($from_date != '' && $to_date != '') {
            $this->db->where("date_of_submission BETWEEN '$start' AND '$end'");
        ...
        $query = $this->db->get();
        foreach ($query->result_array() as $row) {
            $balance += (round($row['cash_account'], 2) * (in_array($this->validation_model->getViewAmountType($row['amount_type']), [
        ...
        if ($query1->num_rows() != 0) {
            foreach ($query1->result_array() as $row) {
                $total_amount = $row['total_amount'] + $row['trans_fee'];
                if ($row['amount_type'] == 'user_credit') {
                    $total_amount = $row['total_amount'];
                }
                $balance += (round($total_amount, 2) * (in_array($row['amount_type'], [
                        'user_debit', 'admin_debit', 'payout_released', 'pin_purchased', 'annual_fee', 'registration', 'repurchase'
                    ]) ? -1 : 1));
            }
        }
        ...
        if ($pin_status) {
            ...
            $query2 = $this->db->get();
            foreach ($query2->result_array() as $row) {
                $balance -= round($row['pin_amount'], 2);
            }
        }
        ...
        $query3 = $this->db->get();
        foreach ($query3->result_array() as $row) {
            $balance -= round($row['paid_amount'], 2);
        }
        $a_type=array('cash','ewallet');
        $this->db->select('user_id,used_amount,used_for,date');
        $this->db->from('ewallet_payment_details');
