<?php
/**
 * @deprecated All uses are commented out
 */
class cash_account_model extends inf_model {
    ...
    public function getCommissionDetails($user_id, $from_date, $to_date) {
        ...
        $this->db->select('amount_payable,total_amount,amount_type,date_of_submission,from_id');
        ...
        $query = $this->db->get();
        foreach ($query->result_array() as $row) {
            $details[$i]['total_amount'] = $row['amount_payable'];
        ...
        $this->db->select('amount as total_amount,date,amount_type,from_user_id,to_user_id,trans_fee');
        $this->db->from('fund_transfer_details');
        $this->db->where('to_user_id', $user_id);
        if ($from_date != '' && $to_date != '') {
...
//new
     public function getCashAccountDetails($user_id, $from_date, $to_date) {
