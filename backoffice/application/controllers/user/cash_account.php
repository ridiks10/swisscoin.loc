...
class cash_account extends Inf_Controller {
    ...
    function my_cash() {
        ...
        $to_date = Date('Y-m-d');
//        $ewallet_details = $this->ewallet_model->getCommissionDetails($user_id, $from_date, $to_date);
//        $details_count = count($ewallet_details);
		...
		$links = $this->inf_pagination->create_links( [
			'base_url'   => base_url() . "user/cash_account/my_cash/",
    ...
    function validate_my_ewallet() {
        $this->form_validation->set_rules('user_name', 'User Name', 'trim|required|strip_tags|xss_clean');
