...
//        $payout_details = $this->bonus_model->getPayoutReleasePercentages($user_id);
        $payout_details = $this->bonus_model->getPayoutReleasePercentages2();
    ...
    function direct_bonus() {
        ...
        $flag = "false";
        $this->load_langauge_scripts();
		if( $this->session->userdata('direct_bonus_chosen_uid') ) {
			$user_id = intval( $this->session->userdata('direct_bonus_chosen_uid') );
...
//            $bonus_type = $this->input->post('bonus_type');
            ...
            if ($user_id) {
                ...
                $flag = "true";
				$this->session->set_userdata('direct_bonus_chosen_uid', $user_id );

            } else {
                $msg = lang('Username_not_Exists');
                $this->redirect($msg, 'bonus/direct_bonus', false);
            }
		...
		$links = $this->inf_pagination->create_links( [
			'base_url'   => base_url() . "admin/bonus/{$bonus_type}/",
    ...
    function matching_bonus() {
        ...
        $flag = "false";
        $this->load_langauge_scripts();
		if( $this->session->userdata('matching_bonus_chosen_uid') ) {
			$user_id = intval( $this->session->userdata('matching_bonus_chosen_uid') );
			$user_name = $this->validation_model->IdToUserName( $user_id );
		} else {
			$user_id = $this->LOG_USER_ID;
			$user_name = $this->LOG_USER_NAME;
...
//            $bonus_type = $this->input->post('bonus_type');
            ...
            if ($user_id) {
                $is_valid_username = true;
                $flag = "true";
				$this->session->set_userdata('matching_bonus_chosen_uid', $user_id );

            } else {
                $msg = lang('Username_not_Exists');
                $this->redirect($msg, 'bonus/matching_bonus', false);
            }
    ...
    function team_bonus() {
       // $title = 'Team Bonus';
        ...
        $flag = "false";
        $this->load_langauge_scripts();
		if( $this->session->userdata('team_bonus_chosen_uid') ) {
			$user_id = intval( $this->session->userdata('team_bonus_chosen_uid') );
			$user_name = $this->validation_model->IdToUserName( $user_id );
		} else {
			$user_id = $this->LOG_USER_ID;
			$user_name = $this->LOG_USER_NAME;
...
//            $bonus_type = $this->input->post('bonus_type');
            ...
            if ($user_id) {
                $is_valid_username = true;
                $flag = "true";
				$this->session->set_userdata('team_bonus_chosen_uid', $user_id );

            } else {
                $msg = lang('Username_not_Exists');
                $this->redirect($msg, 'bonus/team_bonus', false);
            }
    ...
    function fast_start_bonus() {
        ...
        $flag = "false";
        $this->load_langauge_scripts();
		if( $this->session->userdata('fast_start_bonus_chosen_uid') ) {
			$user_id = intval( $this->session->userdata('fast_start_bonus_chosen_uid') );
			$user_name = $this->validation_model->IdToUserName( $user_id );
		} else {
			$user_id = $this->LOG_USER_ID;
			$user_name = $this->LOG_USER_NAME;
		...
		$matching_bonus_details = '';
		$user_type              = $this->LOG_USER_TYPE;
		$flag                   = $user_type == 'employee' ? 'false' : 'true';
...
//            $bonus_type = $this->input->post('bonus_type');
            ...
            if ($user_id) {
                $is_valid_username = true;
                $flag = "true";
				$this->session->set_userdata('fast_start_bonus_chosen_uid', $user_id );

            } else {
                $msg = lang('Username_not_Exists');
                $this->redirect($msg, 'bonus/fast_start_bonus', false);
            }
