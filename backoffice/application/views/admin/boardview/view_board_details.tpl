                ...
                <div class="row">
                    <div class="col-sm-12">
                        ...
                        {foreach from=$board_details item=v}
                            <a id="element_{$v.board_id}" href="{$BASE_URL}admin/boardview/view_board_details/{$v.board_id}"  title="{$langs['show_all']}" >   
                        ...
                        {foreach from=$board_details item=v}
                            <div id ="user_boards{$v.board_id}" {if $v.board_id != $board_no}style="display:none;"{/if} class="user_boards">
                                {if $v.board_id == $board_no}
                                    {include file="admin/boardview/board_view_management.tpl" title="Example Smarty Page" name=""}
                                {/if}
                            </div>
                        {/foreach}
                    </div>
